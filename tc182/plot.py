...
def xy(axes, plots, options):
    ...
    if options['full_title']:
        axes.set_title(
            'CIE xy fundamental chromaticity diagram\nField size: ' +
            str(plots['field_size']) +
            u'\N{DEGREE SIGN},  Age: ' + str(plots['age']) +
...
def lms_base(axes, plots, options):
    ...
    if options['full_title']:
        axes.set_title(
            'CIE 2006 LMS cone fundamentals (9 sign. figs. data)\nField size: '
            + str(plots['field_size']) + u'\N{DEGREE SIGN},  Age: ' +
            str(plots['age']) + u' yr,  Domain: %s\u2013%s nm' %
            (plots['lambda_min'], plots['lambda_max']) +
            ',  Step: %s nm' % plots['lambda_step'], fontsize=12)
    else:
...
def lm(axes, plots, options):
    ...
    lambdavalues = np.concatenate((np.arange(450, 631, 10), [700],
                                   [plots['lm'][0, 0]], [plots['lm'][-1, 0]]))
    for l in lambdavalues:      # add wavelength parameters
